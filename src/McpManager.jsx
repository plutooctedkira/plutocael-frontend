...
export default function McpManager({ colors: C, dark, addSignal }) {
  ...
  const inputStyle = { width: "100%", boxSizing: "border-box", border: `1px solid ${C.divider}`, borderRadius: 12, padding: "11px 13px", fontSize: 15, outline: "none", background: C.input, color: C.text, fontFamily: "inherit" };
    ...
    <>
      <PullRefresh onRefresh={load} color={C.accent} className="panel-scroll" style={{ flex: 1, minHeight: 0, overflowY: "auto", overflowX: "hidden", overscrollBehaviorY: "contain", touchAction: "pan-y", padding: "10px 16px calc(24px + env(safe-area-inset-bottom, 0px))", boxSizing: "border-box" }}>
        <div style={{ maxWidth: 620, margin: "0 auto" }}>
          <div style={{ fontSize: 13, color: C.placeholder, padding: "0 4px 10px" }}>{servers.length} 个服务器 · 聚合 {tools.length} 个工具给 Cael 使用 · 左滑删除</div>

          {servers.length === 0 && <div style={{ background: C.cardBg, borderRadius: 16, padding: 20, textAlign: "center", fontSize: 14, color: C.placeholder, boxShadow: cardShadow }}>还没有 MCP 服务器，点下面添加一个</div>}
              ...
              <SwipeRow onDelete={() => delServer(s)} radius={16}>
                <button className="flat ghost" onClick={() => { setTest(null); setSheet({ id: s.id, name: s.name, url: s.url, enabled: !!s.enabled }); }}
                  ...
                  <span style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ display: "block", fontSize: 16.5, fontWeight: 600, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.name}</span>
                    <span style={{ display: "flex", gap: 6, marginTop: 7, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 12.5, padding: "2px 10px", borderRadius: 11, background: s.enabled ? "rgba(58,175,107,0.13)" : (dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.06)"), color: s.enabled ? "#2E8B57" : C.textSecondary, border: `1px solid ${s.enabled ? "rgba(58,175,107,0.35)" : C.divider}` }}>{s.enabled ? "已连接" : "已停用"}</span>
                      <span style={{ fontSize: 12.5, padding: "2px 10px", borderRadius: 11, background: dark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.05)", color: C.textSecondary, border: `1px solid ${C.divider}` }}>工具: {serverTools.length}/{serverTools.length}</span>
                    </span>
                  </span>
                </button>
              </SwipeRow>
          ...
          <div style={{ fontSize: 13, color: C.placeholder, padding: "12px 4px 0", lineHeight: 1.7 }}>💡 右上角 + 添加。地址填完整的 MCP 端点（Streamable HTTP），鉴权直接拼在 URL 里，例如 https://xxx.com/mcp?api_key=你的key</div>
        </div>
      </PullRefresh>
      ...
      {sheet && <div onClick={() => setSheet(null)} style={{ position: "fixed", inset: 0, zIndex: 620, background: "rgba(0,0,0,0.35)", display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
        <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: 520, background: C.cardBg, borderRadius: "20px 20px 0 0", maxHeight: "86vh", display: "flex", flexDirection: "column", padding: "6px 0 calc(18px + env(safe-area-inset-bottom, 0px))", animation: "slideUp 0.3s cubic-bezier(0.32, 0.72, 0, 1)" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", padding: "4px 16px 10px", flexShrink: 0 }}>
            <button className="flat ghost" onClick={() => setSheet(null)} style={{ width: 32, height: 32, borderRadius: "50%", border: "none", background: "transparent", color: C.textSecondary, cursor: "pointer", fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>✕</button>
            <span style={{ flex: 1, textAlign: "center", fontSize: 16.5, fontWeight: 600, color: C.text }}>{sheet.id ? "编辑 MCP" : "添加 MCP"}</span>
            <span style={{ width: 32 }} />
          </div>
          <div className="panel-scroll" style={{ flex: 1, minHeight: 0, overflowY: "auto", overscrollBehaviorY: "contain", touchAction: "pan-y", padding: "2px 18px 6px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: dark ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.03)", borderRadius: 14, padding: "13px 15px", marginBottom: 14 }}>
              <span style={{ fontSize: 15.5, color: C.text }}>是否启用</span>
            ...
            <div style={{ fontSize: 13.5, color: C.textSecondary, padding: "0 2px 6px" }}>名称</div>
            <input value={sheet.name} onChange={e => setSheet({ ...sheet, name: e.target.value })} placeholder="My MCP" style={{ ...inputStyle, marginBottom: 14 }} />
            <div style={{ fontSize: 13.5, color: C.textSecondary, padding: "0 2px 6px" }}>服务器地址</div>
            ...
            {test && !test.loading && (test.ok
              ? <div style={{ fontSize: 13.5, color: "#2E8B57", background: "rgba(58,175,107,0.12)", borderRadius: 10, padding: "9px 12px", marginBottom: 10 }}>✓ 连接成功，{(test.tools || []).length} 个工具{test.tools && test.tools.length ? "：" + test.tools.slice(0, 6).join("、") + (test.tools.length > 6 ? "…" : "") : ""}</div>
              : <div style={{ fontSize: 13.5, color: "#C0392B", background: "rgba(192,57,43,0.10)", borderRadius: 10, padding: "9px 12px", marginBottom: 10, overflowWrap: "anywhere" }}>✗ 连接失败：{test.error}</div>)}
          </div>
          <div style={{ display: "flex", gap: 10, padding: "10px 18px 0", flexShrink: 0 }}>
            <button className="ghost" onClick={runTest} style={{ padding: "12px 18px", border: `1px solid ${C.divider}`, borderRadius: 14, background: "transparent", color: C.text, cursor: "pointer", fontSize: 15, fontFamily: "inherit" }}>{test && test.loading ? "测试中…" : "测试连接"}</button>
            <button className="ghost" onClick={saveSheet} style={{ flex: 1, padding: "12px", border: "none", borderRadius: 14, background: C.accent, color: "#fff", cursor: "pointer", fontSize: 15, fontWeight: 600, fontFamily: "inherit" }}>✓ 保存</button>
          </div>
        </div>
      </div>}
    </>
