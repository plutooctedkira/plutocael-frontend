...
export default function SwipeRow({ onDelete, children, deleteLabel = "删除", radius = 12 }) {
  ...
  const W = 64;      // 删除按钮露出的宽度
  const OVER = 24;   // 回弹余量：按钮比 W 宽这么多，滑过头也不会露出容器底色
  ...
  return (
    <div style={{ position: "relative", overflow: "hidden", borderRadius: radius }}>
      {/* 比 W 宽 OVER，且靠右贴死：滑过头时露出来的仍是红色而不是容器底色 */}
      <button onClick={() => { setX(0); onDelete && onDelete(); }}
        style={{ position: "absolute", top: 0, right: 0, bottom: 0, width: W + OVER, padding: 0, border: "none", borderRadius: `0 ${radius}px ${radius}px 0`, background: "#D9534F", color: "#fff", fontSize: 14, cursor: "pointer", fontFamily: "inherit", opacity: open ? 1 : 0, display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
        {/* 文字居中在实际露出的那 W 宽度里，多出的 OVER 藏在卡片下面 */}
        <span style={{ width: W, textAlign: "center", flexShrink: 0 }}>{deleteLabel}</span>
      </button>
